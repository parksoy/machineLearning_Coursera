...
% Unroll parameters 
%nn_params = [Theta1(:) ; Theta2(:)]; %25x401, 10x26 therefore 10285X1
...
J = 0;
Theta1_grad = zeros(size(Theta1));%25X401, dJdtheta1
Theta2_grad = zeros(size(Theta2));%10x26, dJdtheta2
...
%J=-1/m*sum(sum(-y_matrix.*log(h)-(1-y_matrix).*log(1-h))); This doesnt work. 
J=(-sum(sum(y_matrix.*log(h))) - sum(sum((1-y_matrix).*(log(1-h)))))/m;

%sum(5000x10),across 5000 cases along the vertical, resulting in 1X10. 
...
% Unroll gradients
grad = [Theta1_grad(:) ; Theta2_grad(:)];%25X401,%10x26
